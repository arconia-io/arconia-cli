...
NOTE: Arconia Dev Services are available for a wide range of services, including data stores (PostgreSQL, MongoDB, Redis, MySQL, and more), message brokers (Kafka, RabbitMQ, Pulsar), observability platforms (Grafana LGTM, OpenTelemetry Collector), and AI inference servers (Ollama). See the https://docs.arconia.io/arconia/latest/dev-services/[Arconia Dev Services documentation] for the full list.

TIP: If you're using Spring AI with a vector database, Arconia can automatically use the appropriate container image. For example, when `arconia-dev-services-postgresql` is present alongside Spring AI PGVector, it automatically uses the `pgvector/pgvector` image.
