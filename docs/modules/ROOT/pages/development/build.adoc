...
NOTE: The `--native` option requires https://www.graalvm.org/[GraalVM] with the Native Image component installed. Native builds take a little bit longer than JVM builds but produce a standalone binary with fast startup time and low memory footprint.
