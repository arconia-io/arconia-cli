...
Run an OpenRewrite recipe to migrate or refactor your project.
...
Discover available OpenRewrite recipes.
...
To discover available recipes, use the `arconia rewrite discover` command or visit the https://docs.openrewrite.org/recipes[OpenRewrite Recipe Catalog]. Use the `--dry-run` option with `arconia rewrite run` to preview changes before applying them.
