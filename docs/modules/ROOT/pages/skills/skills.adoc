...
Skills are always installed into the default `.agents/skills` directory. Optionally, you can also copy them to additional vendor-specific directories using the `--agent` option.

== Multi-Agent Vendor Support

When adding skills with the `--agent` option, the skill is installed to `.agents/skills` and copied to the vendor-specific skills directory. This lets multiple coding agents share the same set of skills without duplicating OCI fetches.
...
The agent configuration is persisted in `skills.json` (via the `additionalBasePaths` field) so that `arconia skills install` reproduces the same layout, and `arconia skills remove` cleans up all copies.
...
Install all agent skills declared in the project's configuration (`skills.json`). If a skill entry includes `additionalBasePaths`, the skill is also copied to those vendor directories.
...
List installed agent skills. When a skill has been copied to additional vendor directories, those paths are shown on continuation lines.
...
Remove agent skills from the project. This deletes the skill from the primary `.agents/skills` directory and any additional vendor directories recorded in the lockfile.
