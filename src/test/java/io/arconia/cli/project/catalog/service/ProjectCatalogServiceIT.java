...
class ProjectCatalogServiceIT extends OciIntegrationTests {
    ...
    void setUp() throws Exception {
        ...
        service = new ProjectCatalogService(registry, createOutputOptions(outWriter, errWriter));
    }
    ...
    @Test
    void addCatalogSavesToRegistryAndCache() throws IOException {
        String catalogRef = pushEmptyCatalog("spring-templates", "1.0.0");

        service.addCatalog("spring-templates", catalogRef);

        Index cached = ProjectCatalogCache.load("spring-templates");
        assertThat(cached).isNotNull();

        ProjectCatalogRegistry.CatalogEntry entry = ProjectCatalogRegistry.findCatalog("spring-templates");
        assertThat(entry).isNotNull();
        assertThat(entry.name()).isEqualTo("spring-templates");
        assertThat(entry.ref()).isEqualTo(catalogRef);
        assertThat(entry.digest()).startsWith("sha256:");
    }

    @Test
    void removeCatalogDeletesFromRegistryAndCache() throws IOException {
        String catalogRef = pushEmptyCatalog("data-templates", "1.0.0");
        service.addCatalog("data-templates", catalogRef);

        service.removeCatalog("data-templates");

        Index cached = ProjectCatalogCache.load("data-templates");
        assertThat(cached).isNull();

        ProjectCatalogRegistry.CatalogEntry entry = ProjectCatalogRegistry.findCatalog("data-templates");
        assertThat(entry).isNull();
    }

    @Test
    void removeCatalogIsIdempotentWhenNotPresent() throws IOException {
        service.removeCatalog("nonexistent-catalog");

        ProjectCatalogRegistry.CatalogEntry entry = ProjectCatalogRegistry.findCatalog("nonexistent-catalog");
        assertThat(entry).isNull();
    }

    @Test
    void updateCatalogRefreshesRegistryAndCache() throws IOException {
        String refV1 = pushEmptyCatalog("update-templates", "1.0.0");
        service.addCatalog("update-templates", refV1);
        ProjectCatalogRegistry.CatalogEntry catalogV1 = ProjectCatalogRegistry.findCatalog("update-templates");
        assertThat(catalogV1).isNotNull();
        String digestV1 = catalogV1.digest();

        String refV2 = pushCatalogWithAnnotations("update-templates", "1.1.0", Map.of("key", "value"));

        service.updateCatalog("update-templates");

        Index cached = ProjectCatalogCache.load("update-templates");
        assertThat(cached).isNotNull();

        ProjectCatalogRegistry.CatalogEntry entry = ProjectCatalogRegistry.findCatalog("update-templates");
        assertThat(entry).isNotNull();
    ...
    @Test
    void updateCatalogDoesNothingWhenNotRegistered() throws IOException {
        service.updateCatalog("nonexistent-catalog");
    ...
    @Test
    void listCatalogDoesNothingWhenNotRegistered() throws IOException {
        service.listCatalog("nonexistent-catalog");

        String errOutput = errWriter.toString();
        assertThat(errOutput).contains("nonexistent-catalog");
    ...
    @Test
    void listCatalogDisplaysEmptyCatalog() throws IOException {
        String catalogRef = pushCatalogWithAnnotations("list-empty", "1.0.0",
                Map.of(ArtifactAnnotations.OCI_DESCRIPTION, "An empty catalog"));
        service.addCatalog("list-empty", catalogRef);
        outWriter.getBuffer().setLength(0);

        service.listCatalog("list-empty");

        String output = outWriter.toString();
        assertThat(output).contains("list-empty");
        assertThat(output).contains(catalogRef);
        assertThat(output).contains("An empty catalog");
    ...
    @Test
    void listCatalogDisplaysCatalogWithProjects() throws IOException {
        String catalogRef = pushCatalogWithProjects("list-projects", "1.0.0");
        service.addCatalog("list-projects", catalogRef);
        outWriter.getBuffer().setLength(0);

        service.listCatalog("list-projects");

        String output = outWriter.toString();
        assertThat(output).contains("list-projects");
        assertThat(output).contains("A catalog with projects");
    ...
    private String pushEmptyCatalog(String name, String tag) {
        String ref = "%s/test/catalogs/%s".formatted(registryUrl, name);
        ContainerRef containerRef = ContainerRef.parse(ref).withTag(tag);
        registry.pushIndex(containerRef, Index.fromManifests(List.of()));
        return ref + ":" + tag;
    }

    private String pushCatalogWithAnnotations(String name, String tag, Map<String, String> annotations) {
        String ref = "%s/test/catalogs/%s".formatted(registryUrl, name);
        ContainerRef containerRef = ContainerRef.parse(ref).withTag(tag);
    ...
    private String pushCatalogWithProjects(String catalogName, String tag) throws IOException {
        Path contentDir = tempDir.resolve("project-content");
        Files.createDirectories(contentDir);
        Files.writeString(contentDir.resolve("README.md"), "# Test project");

        // Push project manifests into the same repository as the catalog
        // (OCI registries require referenced manifests to exist in the same repo)
        String catalogRepo = "test/catalogs/%s".formatted(catalogName);
        ArtifactPublisher publisher = new ArtifactPublisher(registry);

        Manifest manifest1 = publisher.publish(buildProjectArtifactInfo(contentDir, catalogRepo,
                "my-api", "A REST API service", "service", List.of("java", "spring"), "my-api-1.0.0"));
        Manifest manifest2 = publisher.publish(buildProjectArtifactInfo(contentDir, catalogRepo,
        ...
        // Push the catalog index referencing the real manifests
        String ref = "%s/%s".formatted(registryUrl, catalogRepo);
        ContainerRef containerRef = ContainerRef.parse(ref).withTag(tag);
        Index index = Index.fromManifests(List.of(project1, project2))
                .withAnnotations(Map.of(ArtifactAnnotations.OCI_DESCRIPTION, "A catalog with projects"));
